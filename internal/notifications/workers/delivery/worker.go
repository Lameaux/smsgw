...
func NewWorker(
	name string, app *config.App, messageType models.DeliveryNotificationType, processor notifications.Processor,
) *Worker {
	return &Worker{
		name:        name,
		app:         app,
...
func (w *Worker) Run() (bool, error) {
	...
	notification, err := notifications.NewRepo(tx).FindOneForProcessing(w.messageType)
	if err != nil {
...
func (w *Worker) processNotification(db db.Conn, n *models.DeliveryNotification) error {
	...
	return notifications.NewRepo(db).Update(n)
}

func (w *Worker) handleFailure(n *models.DeliveryNotification, resp *nm.Response, err error) {
...
func (w *Worker) tryReschedule(n *models.DeliveryNotification) {
	if n.AttemptCounter >= MaxAttempts() {
		n.Status = models.DeliveryNotificationStatusFailed
...
func (w *Worker) handleSuccess(n *models.DeliveryNotification, resp *nm.Response) {
